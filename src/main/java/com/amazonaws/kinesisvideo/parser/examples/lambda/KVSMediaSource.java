...
public class KVSMediaSource implements MediaSource {
    ...
    public void putFrameData(EncodedFrame encodedFrame) {
        ...
        if (encodedFrame.getByteBuffer() != null) {
            KinesisVideoFrame frame = new KinesisVideoFrame(this.frameIndex++, flags, encodedFrame.getTimeCode(), encodedFrame.getTimeCode(), 200000L, encodedFrame.getByteBuffer());
            if (frame.getSize() == 0) {
                return;
            }
